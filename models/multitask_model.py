...
class MultiTaskModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.router = DynamicRouter(
            self.backbone.feature_dim, config.num_experts, config.hidden_dim
        )
        ...
        self.experts = nn.ModuleList(
            [
                ...
                SegmentationExpert(self.backbone.feature_dim, config.dropout_rate),
                GeneralExpert(
                    self.backbone.feature_dim, config.hidden_dim, config.dropout_rate
                ),
            ]
        )
            ...
            for i, expert in enumerate(self.experts):
                if i == 1:  # Segmentation expert
                    ...
                    if seg_output is None:
                        seg_output = (
                            routing_weights[:, i : i + 1].unsqueeze(-1).unsqueeze(-1)
                            * expert_output
                        )
                    else:
                        seg_output += (
